...
import { getDefaultNftInfo, NftInfo } from "../../types/NFT";

interface ImageSelectorProps {
  selected: NftInfo;
  images: NftInfo[];
  onSelected: (nft: NftInfo) => void;
...
const ImageSelector = (props: ImageSelectorProps) => {
  ...
  const [hovered, setHovered] = useState(getDefaultNftInfo());

  const handleSelect = (id: NftInfo) => {
    onSelected(id);
  };
    ...
    <ImageSelectorContainer>
      {images.map((i, index) => (
        <ImageContainer
          ...
          onMouseLeave={() => setHovered(getDefaultNftInfo())}
