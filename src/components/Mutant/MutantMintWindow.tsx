...
import { useMoaycWhitelistMint } from "../../hooks/contract/moayc-mint/useMoaycWhitelistMint";
...
const MutantMintWindow = () => {
  ...
  } = useMoaycWhitelistMint(
    saleInfo.currentPrice,
    selectedAmount,
    saleInfo.whitelistMint
  );
    ...
    <>
      {!saleInfo.mutation && (
        <MutationWindowContainer>
          ...
          <MintMenu>
            {saleInfo.whitelistMint && (
              <MintStatus>whitelist mint is live</MintStatus>
            )}
            {saleInfo.publicMint && (
              <MintStatus>public mint is live</MintStatus>
            )}
            ...
            <MutationStatLine
              name={"Price:"}
              value={`${saleInfo.currentPrice} $OP`}
            />
            <MutationStatLine
              name={"Your Allocation:"}
              value={`${getMinted()} / ${getAlloc()}`}
            />
            ...
            />
            {canMint ? (
              <MoaycRectButton style={{ marginTop: 11 }} onClick={handleMint}>
                Mint
              </MoaycRectButton>
            ) : (
              <MoaycRectButton
                style={{ marginTop: 11 }}
                disabled={!isAllocationOk()}
