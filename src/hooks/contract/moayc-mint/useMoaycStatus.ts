...
import { SaleStatus } from "../../../web3/types/SaleStatus";
...
export const useMoaycStatus = () => {
  ...
  const pricePerTokenWl = saleInfo?.pricePerTokenWl
    ? ethers.utils.formatEther(saleInfo?.pricePerTokenWl)
    : 0;
  const pricePerTokenPublic = saleInfo?.pricePerTokenPublic
    ? ethers.utils.formatEther(saleInfo?.pricePerTokenPublic)
    : 0;
  const mutationPrice = saleInfo?.mutationPrice
    ? ethers.utils.formatEther(saleInfo?.mutationPrice)
    : 0;
